...
# Lazy load Q-table from DB (only when needed)
_q_loaded = False

def _load_q_table():
    """Lazy load Q-table from database"""
    global Q, _q_loaded
    if _q_loaded:
        return
    ...
    try:
        ...
        print(f"DEBUG: Could not load Q-table from DB: {e}. Using empty Q-table.")
        Q = np.zeros((n_states, n_actions))
        _q_loaded = True

def save_q_table():
    # Ensure Q-table is loaded first
    _load_q_table()
