...
async def submit_quiz(
    ...
    asyncio.create_task(bucket_client.log_quiz_event(
        user_id=str(current_user.id),
        quiz_id=request.quiz_id,
        score=score_percentage,
        total_questions=len(questions),
        metadata={"subject": quiz_data["subject"], "topic": quiz_data["topic"]}
    ))
    asyncio.create_task(karma_client.log_quiz_completion(
        user_id=str(current_user.id),
        quiz_id=request.quiz_id,
        score=correct_count,
        total=len(questions)
