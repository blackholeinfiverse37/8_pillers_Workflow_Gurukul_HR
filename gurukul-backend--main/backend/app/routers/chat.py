...
from app.services.bucket_client import bucket_client
from app.services.karma_client import karma_client
...
class ChatRequest(BaseModel):
    ...
    use_core: bool = True  # Enable Core integration by default
...
async def chat_endpoint(request: ChatRequest):
    """
    Chat Endpoint with Core Integration + RAG support
    
    Flow:
    1. Try Core integration (routes through Insight Flow → Core agents)
    2. Fallback to local Groq + Knowledge Base if Core unavailable
    """
    conversation_id = request.conversation_id or str(uuid.uuid4())
    
    # Try Core integration first (if enabled)
    if request.use_core and settings.GROQ_API_KEY:
        try:
            core_result = await core_client.process_query(
            ...
            if core_result["success"]:
                agent_output = core_result["data"].get("agent_output", {})
                response_text = agent_output.get("response", agent_output.get("answer", "No response"))
                ...
                asyncio.create_task(bucket_client.log_chat_event(
                    user_id=request.user_id or "anonymous",
                    message=request.message,
                    response=response_text,
                    agent_used=core_result.get("agent_used", "edumentor_agent")
                ))
                asyncio.create_task(karma_client.log_action(
                    user_id=request.user_id or "anonymous",
                    action="asking_questions",
                    role="learner",
                    note=f"Chat query: {request.message[:50]}..."
                ))
                ...
                return {
                    ...
                    "core_used": True,
                    "agent_used": core_result.get("agent_used"),
                    "knowledge_base_used": agent_output.get("knowledge_base_results", 0) > 0,
                    "sources": agent_output.get("sources", [])
                }
        except Exception as e:
            logger.warning(f"Core integration failed, using local fallback: {e}")
