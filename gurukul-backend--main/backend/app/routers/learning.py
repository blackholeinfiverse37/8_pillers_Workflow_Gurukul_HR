...
async def subject_explorer(
    ...
    asyncio.create_task(bucket_client.log_lesson_event(
        user_id=str(current_user.id),
        lesson_id=f"{request.subject}_{request.topic}",
        event_type="explored",
        metadata={"subject": request.subject, "topic": request.topic, "kb_used": kb_used}
    ))
    asyncio.create_task(karma_client.log_action(
        user_id=str(current_user.id),
        action="exploring_topics",
        role="learner",
        note=f"Explored {request.subject}: {request.topic}"
    ))
